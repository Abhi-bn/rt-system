...
Cache::Cache(int l_, long int size_) : l(l_), size(size_) {
    ...
    size = max(size,
               2 * (long int)l);  // cache must be large enough for two columns
...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param) {
    ...
    if (param->svm_type == ONE_CLASS || param->svm_type == EPSILON_SVR ||
        ...
        if (nr_class == 1)
            info(
                "WARNING: training data in only one class. See README for "
                "details.\n");
...
double svm_get_svr_probability(const svm_model *model) {
    if ((model->param.svm_type == EPSILON_SVR ||
        ...
        fprintf(stderr,
                "Model doesn't contain information for SVR probability "
                "inference\n");
        return 0;
    }
}
...
bool read_model_header(FILE *fp, svm_model *model) {
    ...
    // parameters for training only won't be assigned, but arrays are assigned
    // as NULL for safety
    param.nr_weight = 0;
